...
if (strpos($region, '市') !== false) {
    $region = substr($region, 0, strpos($region, '市') + 3); // 保留到“市”字（包括“市”）
}
...
//判断，如果Country为中国且位于大陆则不显示中国
$excludeChina = (strpos($region, '香港') !== false || strpos($region, '澳门') !== false || strpos($region, '台湾省') !== false);//合规化：位置判断
if ($country != "中国") {
    $welcomeText = '来自'.$country.'-'.$region.'的朋友,欢迎您';
} else {
    $welcomeText = '来自'.$region.'的朋友,欢迎您'; // 国内IP不显示中国
}
//输出
